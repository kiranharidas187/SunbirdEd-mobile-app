...
export class AdminHomePage implements OnInit, OnDestroy, OnTabViewWillEnter {
  ...
  ngOnInit() {
    ...
    this.getRecommandedMentors();
    this.getRecommandedSessions();
  }

  getRecommandedMentors() {
    try{
      this.common.getRecommendedData({type:"mentor"}).subscribe(data => {
        this.unformatedMentorList = data['data'].mentors
        this.formatMentorList();
      })
    }catch(e){}
  }

  getRecommandedSessions() {
    try{
      this.common.getRecommendedData({type:"session"}).subscribe(data => {
        this.unformatedSessionList = data['data'].sessions
        this.formatSessionList();
      })
    }catch(e){}
  }
  ...
  formatSessionList() {
    let index = 0;
    for (const session of this.unformatedSessionList) {
      const obj = {
        index: index,
        courseLogoUrl: "",
        courseId: session?.item?.id,
        courseName: session?.item?.title,
        cardImg: session?.item?.image,
        content: {
          orgDetails: {
            orgName: session?.item?.description
          }
        }
      }
  ...
  naviteToSessionDetails(event) {
    const index = event.data.index;
    this.router.navigate(['/mentoring/session-details/'+ event?.data?.courseId])
  }
  ...
  async ionViewWillEnter() {
    if(!this.unformatedMentorList?.length){
      this.getRecommandedMentors();
    }
    if(!this.unformatedSessionList?.length){
      this.getRecommandedSessions();
    }
